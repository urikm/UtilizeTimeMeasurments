...
def CreateMMTrajectory(mu, F, nTimeStamps, fullCg=False, isCG=True, remap=False):
    ...
    if isCG:
        ...
        if fullCg or (not fullCg and remap):
            ...
            trainBuffer = np.ones(mCgTrajectory[:, 0].size)  # used only in full-cg
...
for ix, x in enumerate(vGridInterp):
    ...
    mCgTraj, nCgDim, vHiddenStatesF, _ = CreateMMTrajectory(mu, F, int(trajlength), fullCg=False, isCG=False, remap=False)  # TODO : TODEL
    vFull[ix] = EntropyRateCalculation(nDim, mWx, vPiX) * mCgTraj[:, 1].mean()
