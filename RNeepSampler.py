...
class CartesianSeqSampler(CartesianSampler):
    def __init__(self, M, L, n, batch_size,  trainIter, device="cpu", train=True):
        ...
        Attributes:
            ...
            trainIter: number of train iterations
            train: if True randomly sample a subset else ordered sample. (default: True)
    ...
    def __next__(self):
        if self.training:
            ...
            if isLast:
                ...
                traj_idx = self.trj_idx + traj_idx
            self.index += 1
            batch = traj_idx.transpose(0,1)
            return batch
        
    def __len__(self):
        if self.training:
            return self.trainIter
        else:
            return int(self.dropL/self.batch_size/self.seq_length)
