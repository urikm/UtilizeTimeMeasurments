...
# %% Mapping from state to indices - done by ascending order. This is done in order to avoid ambiguity in rate matrix
def MapStates2Indices(vStates, states2Omit=[]):
    dMap = {}
    vEffStates = vStates
    # Omit unwanted states
    for omitState in states2Omit:
        vEffStates = np.delete(vEffStates, np.where(vEffStates == omitState))

    vEffStates = np.sort(vEffStates)
